...
def _update_builder(env, builder):
    ...
    def new_emitter(target, source, env):
        ...
        # On Windows, we don't want to emit the PDB. Something else is
        # already doing so, so otherwise it wouold be emitted
        # twice. This tool only needed to adorn the node with the
        # various attributes as needed. Just return the original
        # target, source tuple.
